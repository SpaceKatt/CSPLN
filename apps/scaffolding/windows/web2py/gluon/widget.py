...
----------------------------------
----------------------------------
 File modified by Joseph J. Simpson 07/19/2015
 To support application start without admin and network parameters input
...
ProgramName = 'CSPLNB_MKE'
ProgramAuthor = 'Based on code created by Massimo Di Pierro, Copyright 2007-' + str(
...
def get_url(host, path='/', proto='http', port=80):
    ...
    return 'http://127.0.0.1:8000/MKE_Static_Name/default/index'

#def start_browser(url, startup=False):
url_s = 'http://127.0.0.1:8000/MKE_Static_Name/default/index'
def start_browser(url_s, startup=False):
    if startup:
        print 'please visit:'
        print '\t', url_s # note change here
        print 'starting browser...'
    try:
        import webbrowser
        webbrowser.open(url_s) #be sure to note change here
    except:
...
class web2pyDialog(object):
    ...
    def __init__(self, root, options):
        ...
        iconphoto = os.path.join('extras', 'icons', 'ita.gif')
        ...
        logo = os.path.join('extras', 'icons', 'ita.gif')
        ...
        #              justify=Tkinter.RIGHT).grid(row=shift,                                              column=1, pady=10,
        #                                          sticky=sticky)
